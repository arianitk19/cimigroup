...
(function () {
  ...
  function show(i, instant) {
    ...
    var it = items[index];
    if (!instant) lb.classList.add('is-swapping');
    var apply = function () {
      lbImg.onload = function () { lb.classList.remove('is-swapping'); };
      lbImg.onerror = function () { lbImg.onerror = null; lbImg.src = PH[it.ratio] || PH.wide; };
      lbImg.alt = it.caption || '';
      lbImg.src = it.src;
      // if cached and already complete, clear swapping immediately
      if (lbImg.complete && lbImg.naturalWidth) lb.classList.remove('is-swapping');
    ...
    if (instant) apply(); else setTimeout(apply, 120);
  }
  function openLB(list, i) {
    if (!list || !list.length) return;
    items = list; lastFocus = document.activeElement; lb.hidden = false;
    requestAnimationFrame(function () { lb.classList.add('is-open'); });
    document.body.classList.add('no-scroll'); show(i || 0, true);
  ...
  function renderProject(p) {
    ...
    ovContent.innerHTML = '<div class="panel-hero">' + H.img(p.image, L(p.title), p.ratio) + '</div><div class="panel-body"><span class="panel-cat">' + esc(L(p.category)) + '</span><h2 class="panel-title">' + esc(L(p.title)) + '</h2>' + (meta.length ? '<div class="panel-meta">' + meta.join('') + '</div>' : '') + '<p class="panel-desc">' + esc(L(p.description) || L(p.excerpt)) + '</p>' + mats + gal + '</div>';
  }
  function openProject(p) {
    ovFocus = document.activeElement; renderProject(p); ov.hidden = false;
